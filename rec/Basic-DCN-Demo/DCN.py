    ...
    def _init_graph(self):
        ...
        with self.graph.as_default():
            ...
            for l in range(self.cross_layer_num):
                # todo multiply 用于计算矩阵之间的element-wise 乘法，要求矩阵的形状必须一致；
                #  matmul 是tensor的矩阵乘法, 参与运算的两个tensor维度、数据类型必须一致
                #  tensordot:矩阵乘法运算,参与运算的两个tensor的维度可以不一样:https://blog.csdn.net/qq_28966711/article/details/89606297
                x_l = tf.tensordot(tf.matmul(self._x0, x_l, transpose_b=True),
                                    self.weights["cross_layer_%d" % l],1) + self.weights["cross_bias_%d" % l] + x_l
            # print(f"x_l={x_l}")
            # todo x_l=Tensor("add_7:0", shape=(?, 249, 1), dtype=float32)
            self.cross_network_out = tf.reshape(x_l, (-1, self.total_size))
            # todo self.cross_network_out=Tensor("Reshape_3:0", shape=(?, 249), dtype=float32)
            # print(f"self.cross_network_out={self.cross_network_out}")
