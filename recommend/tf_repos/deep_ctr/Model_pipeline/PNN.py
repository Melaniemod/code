...
def input_fn(filenames, batch_size=32, num_epochs=1, perform_shuffle=False):
    ...
    def decode_libsvm(line):
        ...
        # todo 这里 id_vals 是什么形式为什么下面可以直接对 id_vals 直接分割？
        ...
        # todo columns=SparseTensor(indices=Tensor("StringSplit/StringSplit:0", shape=(?, 2), dtype=int64), values=Tensor("StringSplit/StringSplit:1",
        #  shape=(?,), dtype=string), dense_shape=Tensor("StringSplit/StringSplit:2", shape=(2,), dtype=int64));
        #  labels=Tensor("StringToNumber:0", shape=(), dtype=float32)
        #  splits=SparseTensor(indices=Tensor("StringSplit_1/StringSplit:0", shape=(?, 2), dtype=int64), values=Tensor("StringSplit_1/StringSplit:1",
        #  shape=(?,), dtype=string), dense_shape=Tensor("StringSplit_1/StringSplit:2", shape=(2,), dtype=int64));
        #  splits.dense_shape=Tensor("StringSplit_1/StringSplit:2", shape=(2,), dtype=int64)
        # print(f"columns={columns}; labels={labels}; splits={splits}; splits.dense_shape={splits.dense_shape}")
    ...
    #  dataset = dataset.shuffle(buffer_size):buffle的机制是在内存缓冲区中保存一个buffer_size条数据，每读入一条数据后，从这个缓冲区中随机选择一条数据进行输出，缓冲区的大小越大，随机的性能就越好，但是也更耗费内存。
...
def model_fn(features, labels, mode, params):
    ...
    #------build f(x)------
    # todo 这里线性部分，是用onehot特征还是用 embedding 相乘得到的？看到论文上是用的 embedding 相乘得到的吧 -- 不是的，是用field 权重矩阵 和索引得到的。
    ...
    with tf.variable_scope("Product-layer"):
        if FLAGS.model_type == 'FNN':
            ...
            # todo einsum('ij,jk->ik', m0, m1)  # output[i,k] = sum_j m0[i,j] * m1[j, k]
            #  #  Dot product
